...
| **1 — Build the actual safety product** | SOS, medical info, family circle, and guidance work end-to-end on-device, no backend needed. | ✅ **Complete (2026-09-10).** Real maps (MapLibre + OpenFreeMap) shipped, closing the phase's last open item. **2026-09-11: web maps also shipped** (MapLibre GL JS v5 + the same OpenFreeMap tiles) — see §3.1. |
...
- **Follow-up (2026-09-11): web maps shipped — the dedicated session
  flagged above.** User reported the web build showing the static-pin
  fallback and asked for it fixed, i.e. asked for the deferred work
  above. Confirmed both real risks that entry flagged, then resolved
  them rather than working around them:
  - **SDK version pinned to the v5 line, not latest (v6.9.0).** Checked
    MapLibre's own v5-to-v6 migration guide: v6 moved to an ESM-only
    build whose Web Worker has to be served as its own bundled asset via
    a bundler-specific import (Vite's `?worker&url` query, or an
    equivalent for webpack/rspack) — Metro's web bundler has no
    equivalent mechanism, and getting one working was exactly the "Web
    Worker URL registration quirk" risk this entry called out. v5 is the
    last release with a self-contained UMD/CJS bundle whose worker is
    built from an inline Blob URL at runtime, so it needs zero
    bundler-specific wiring — confirmed with `node -e "require('maplibre-gl')"`
    that it loads cleanly with plain CJS `require()`, and the *same*
    OpenFreeMap style URL already in use works unchanged on this SDK
    too. Installed via plain `npm install maplibre-gl@^5.24.0` (the
    `expo install` network-version-check failure is the same known class
    of issue as `@maplibre/maplibre-react-native` above, not a new one).
  - **The CSS-import risk was sidestepped rather than resolved** —
    `maplibre-gl/dist/maplibre-gl.css` is never imported at all. It only
    styles chrome this component doesn't use (default marker SVGs, since
    markers are custom elements; nav/geolocate controls, since none are
    added; popups, since none are used). The two rules it would actually
    have contributed — a grab cursor and `touch-action: none` on the
    canvas — are injected directly as a two-line `<style>` tag instead,
    which avoids needing to verify a node_modules CSS import against
    this project's Metro web-export setup specifically, per the original
    entry's stated concern.
  - **Split `MiniMap.tsx` by platform** instead of keeping the old
    single-file `Platform.OS === 'web' ? … : null` branch:
    `src/components/MiniMap.shared.tsx` (marker type, coordinate
    validation, the static-pin fallback, pin badge color logic — used by
    both), `MiniMap.tsx` (native, unchanged behavior, now just requires
    `@maplibre/maplibre-react-native` directly since this file is
    native-only by construction), `MiniMap.web.tsx` (new — real
    MapLibre GL JS map). Metro/Expo Router's standard `.web.tsx`
    platform-extension resolution picks the right file per platform;
    confirmed by grepping the exported web bundle — it now contains
    `maplibre-gl` (web SDK) and **zero** occurrences of
    `@maplibre/maplibre-react-native`/`MLRNCameraModule`, where the old
    single-file version bundled the native module's code into the web
    build unconditionally (harmless there since it's apparently
    web-stubbed, but this is strictly cleaner and smaller). Jest is
    unaffected — `jest-expo` reports `Platform.OS === 'ios'` by default
    (per the existing mock's own comment above), so it resolves
    `MiniMap.tsx`, not the `.web.tsx` sibling, same as before.
  - Web markers are plain DOM elements built to match
    `MiniMap.shared.tsx`'s `MapPinBadge` visuals exactly (same
    background/border colors by `kind`, same lucide `map-pin` glyph
    inlined as a static SVG string) rather than mounting a second React
    tree just to reparent a node into MapLibre's marker wrapper.
  - Camera framing differs slightly from native on purpose: web uses a
    real `fitBounds` (trivial here since it's one imperative call on the
    vanilla JS API, unlike native's declarative `Camera` component) for
    2+ markers instead of native's marker-average-center tradeoff — a
    genuine small improvement, not a bug needing backport; native's
    reasoning in §3.1 still stands for why it isn't worth the extra
    camera-ref plumbing there.
  Verified: `tsc --noEmit` clean, **51/51 tests still passing**, a real
  `npx expo export --platform web` succeeds (all 34 routes bundle,
  5.5MB entry bundle), and the bundle grep above. Not verified: actual
  pixel rendering in a real browser (no browser available in this
  sandbox) — the fix is verified at the build/bundle level, not visually
  confirmed; ask the user to check `localhost:8081/safe` after this
  lands.
...
- [x] **Real maps.** **2026-09-10**: Shipped per §3.1's plan — MapLibre
  ...
  **Deliberately not done, matching §3.1's original scope line:** no web
  ...
  web is a nice-to-have only) — **shipped 2026-09-11, see that date's
  entry above and §3.1's "Web maps" note; used plain `maplibre-gl`
  directly rather than `react-map-gl`.** No map on `sos.tsx`'s confirmed screen
...
**Web maps — shipped 2026-09-11, closing the scope line above.** See the
dated log entry under Phase 1 (2026-09-11) for the full writeup —
`maplibre-gl` v5 (not v6 — ESM-only worker wiring isn't Metro-compatible,
see that entry), no `maplibre-gl.css` import, `MiniMap.tsx` split into
`MiniMap.shared.tsx` + `MiniMap.tsx` (native) + `MiniMap.web.tsx` (web) so
Metro's platform-extension resolution picks the right one instead of a
runtime `Platform.OS` branch. `sos.tsx`'s map is still the one open
stretch item, unaffected by this.
